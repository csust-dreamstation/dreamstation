...
(function () {
    ...
    URL = window.UEDITOR_HOME_URL||"/DreamStation/editor/";//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
    ...
    window.UEDITOR_CONFIG = {
        ...
        //图片上传配置区
        ,imageUrl:URL+"FileUploadAction"             //图片上传提交地址
        ,imagePath:URL + ""                     //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
       //,imageFieldName:"upfile"                   //图片数据的key,若此处修改，需要在后台对应文件修改对应参数
        //,compressSide:0                            //等比压缩的基准，确定maxImageSideLength参数的参照对象。0为按照最长边，1为按照宽度，2为按照高度
        ,maxImageSideLength:450                    //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值，更多设置在image.html中

        //涂鸦图片配置区
        ,scrawlUrl:URL+"ScrawlAction"           //涂鸦上传地址
        ,scrawlPath:URL+""                            //图片修正地址，同imagePath

        //附件上传配置区
        ,fileUrl:URL+"FileAction"               //附件上传提交地址
        ,filePath:URL + ""                   //附件修正地址，同imagePath
        //,fileFieldName:"upfile"                    //附件提交的表单名，若此处修改，需要在后台对应文件修改对应参数

         //远程抓取配置区
        //,catchRemoteImageEnable:true               //是否开启远程图片抓取,默认开启
        ,catcherUrl:URL +"RemoteImageAction"   //处理远程图片抓取的地址
        ...
        //图片在线管理配置区
        ,imageManagerUrl:URL + "ImageManagerAction"       //图片在线管理的处理地址
        ,imageManagerPath:URL + ""                                    //图片修正地址，同imagePath

        //屏幕截图配置区
        ,snapscreenHost: '127.0.0.1'                                  //屏幕截图的server端文件所在的网站地址或者ip，请不要加http://
        ,snapscreenServerUrl: URL +"FileUploadAction" //屏幕截图的server端保存程序，UEditor的范例代码为“URL +"server/upload/jsp/snapImgUp.jsp"”
        ,snapscreenPath: URL + ""
        //,snapscreenServerPort: 80                                    //屏幕截图的server端端口
        //,snapscreenImgAlign: 'center'                                //截图的图片默认的排版方式

        //word转存配置区
        ,wordImageUrl:URL + "FileUploadAction"             //word转存提交地址
        ,wordImagePath:URL + ""                       //
        //,wordImageFieldName:"upfile"                     //word转存表单名若此处修改，需要在后台对应文件修改对应参数

        //获取视频数据的地址
        ,getMovieUrl:URL+"MovieAction"                   //视频数据获取地址

        //工具栏上的所有的功能按钮和下拉框，可以在new编辑器的实例时选择自己需要的从新定义
        ,toolbars:[
                   ['fullscreen', 'source', '|',
                    'bold', 'italic', 'underline','autotypeset','blockquote', '|', 'forecolor',  'insertorderedlist', 'insertunorderedlist','selectall', '|',
                    'lineheight',
                    'paragraph', 'fontfamily', 'fontsize', '|',
                    'justifyleft', 'justifycenter', 'justifyright', 'justifyjustify', '|',
                    'link', 'unlink', '|', 'imagenone', 'imageleft', 'imageright','imagecenter', '|',
                    'insertimage', 'emotion','scrawl', 'insertvideo','template','|',
